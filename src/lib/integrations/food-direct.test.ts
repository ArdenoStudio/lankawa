...
// Fuel / non-food rows must be dropped at parse time.
assert.ok(rows.every((row) => !/fuel/i.test(row.commodity)));
assert.ok(rows.every((row) => !/^non-food/i.test(row.category)));
assert.equal(rows.length, 10);
assert.ok(rows.every((row) => row.price > 0));
assert.ok(rows.some((row) => /^eggs$/i.test(row.commodity)));
...
assert.ok(rice);
// Prefer white rice over red nadu; mean of Colombo 270.5 and Gampaha 269.5.
...
assert.ok(
  (snapshot.staleStapleCount ?? 0) >= 1,
  "staleStapleCount reflects old sugar",
);
