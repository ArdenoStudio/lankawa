"""Minimal unofficial HTTP client for OpenAQ Sri Lanka.
...
DEFAULT_UA = "openaq-lk-docs-unofficial/0.1 (+https://github.com/Cookie-Cat21/openaq-lk-docs; educational; polite)"
DEFAULT_BASE = "https://api.openaq.org"


class OpenaqLkDocsClient:
    """Unofficial client — OpenAQ Sri Lanka."""
    ...
    def __enter__(self) -> "OpenaqLkDocsClient":
        return self
    ...
    def locations_lk(self, page: int = 1, limit: int = 50, **kwargs: Any) -> Any:
        """LK monitoring locations (country id may change).

        Catalog id: `locations_lk` · GET
        """
        url = self._with_query("https://api.openaq.org/v3/locations?countries_id=207&limit=100", {'page': page, 'limit': limit})
        payload: Any = None
