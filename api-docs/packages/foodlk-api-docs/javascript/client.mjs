/**
 * Unofficial JavaScript (ESM) client — FoodLK / Food Platform API
...
export class FoodlkApiDocsClient {
  ...
  static slug = "foodlk-api-docs";
  static title = "FoodLK / Food Platform API";
  ...
  async openapi(options = {}) {
    return this.requestJson(this.resolveUrl("https://food-platform-backend.fly.dev/openapi.json", options.query), { ...options, method: "GET" });
  }

  /** Often 200 when hub/summary is 500. */
  async hubManifest(options = {}) {
    return this.requestJson(this.resolveUrl("https://food-platform-backend.fly.dev/api/v1/hub/manifest", options.query), { ...options, method: "GET" });
  }

  /** Preferred Lankawa surface — frequently 500. */
  async hubSummary(options = {}) {
    return this.requestJson(this.resolveUrl("https://food-platform-backend.fly.dev/api/v1/hub/summary", options.query), { ...options, method: "GET" });
  }

  /** Essentials staples preset. */
  async basketEstimate(options = {}) {
    return this.requestJson(this.resolveUrl("https://food-platform-backend.fly.dev/api/v1/basket/estimate?preset=essentials", options.query), { ...options, method: "GET" });
  }
}
