# Client extras — `foodlk-api-docs`

## Typed models

Canonical dataclass / TS types from field-coverage domains: `food_prices`, `platform_misc`.

- Python: `python/foodlk_api_docs/models.py`
...
```python
from foodlk_api_docs import FoodlkApiDocsClient, iter_lab_endpoint

with FoodlkApiDocsClient() as client:
    for page in iter_lab_endpoint(client, None, max_pages=3, page_size=50):
        print(page.page, len(page.items), page.done)
```

```ts
import { FoodlkApiDocsClient, iterLabEndpoint } from "./src/index.js";
const client = new FoodlkApiDocsClient();
...
```python
from foodlk_api_docs import shard_groups, shard_page_numbers, shard_offsets
