/**
 * Unofficial TypeScript client — CEB Care API
 *
...
export class CebcareApiDocsClient {
  ...
  static readonly slug = "cebcare-api-docs";
  static readonly title = "CEB Care API";
   ...
   * Catalog id: `demand_mgmt_schedule` · GET
   */
  async demandMgmtSchedule(options: RequestOptions = {}): Promise<unknown> {
    return this.requestJson<unknown>(this.resolveUrl("https://cebcare.ceb.lk/Incognito/DemandMgmtSchedule", options.query), { ...options, method: "GET" });
  }

  /**
   * Clusters for group A–Y; requires verification token.
   * Catalog id: `get_demand_mgmt_clusters` · GET
   */
  async getDemandMgmtClusters(groupId = "A", options: RequestOptions = {}): Promise<unknown> {
    return this.requestJson<unknown>(this.withQuery("https://cebcare.ceb.lk/Incognito/GetDemandMgmtClusters?LoadShedGroupId=A", { LoadShedGroupId: groupId }, options.query), { ...options, method: "GET" });
  }
}
