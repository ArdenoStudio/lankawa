/**
 * Unofficial TypeScript client — Promise.lk tenders
 *
...
const DEFAULT_UA = "promise-lk-tenders-docs-unofficial/0.1 (+https://github.com/Cookie-Cat21/promise-lk-tenders-docs; educational; polite)";

function sleep(ms: number): Promise<void> {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

export class PromiseLkTendersDocsClient {
  ...
  constructor(options: ClientOptions = {}) {
    this.baseUrl = (options.baseUrl ?? "https://promise.lk").replace(/\/$/, "");
  ...
  static readonly slug = "promise-lk-tenders-docs";
  static readonly title = "Promise.lk tenders";
  ...
  /**
   * Public procurement listings (scrape/API as probed).
   * Catalog id: `procurements_list` · GET
   */
  async procurementsList(options: RequestOptions = {}): Promise<unknown> {
    return this.requestJson<unknown>(this.resolveUrl("https://promise.lk/", options.query), { ...options, method: "GET" });
  }
}

export default PromiseLkTendersDocsClient;
