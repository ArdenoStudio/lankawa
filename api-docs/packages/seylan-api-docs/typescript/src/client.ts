/**
 * Unofficial TypeScript client — Seylan Bank API
 *
...
export class SeylanApiDocsClient {
  ...
  static readonly slug = "seylan-api-docs";
  static readonly title = "Seylan Bank API";
   ...
   */
  async exchangeRatesUsd(options: RequestOptions = {}): Promise<unknown> {
    return this.requestJson<unknown>(this.resolveUrl("https://www.seylan.lk/api/exchange-rates-get-value/USD", options.query), { ...options, method: "GET" });
  }
   ...
   */
  async getFdData(options: RequestOptions = {}): Promise<unknown> {
    return this.requestJson<unknown>(this.resolveUrl("https://www.seylan.lk/get-fd-data", options.query), { ...options, method: "GET" });
  }
}
