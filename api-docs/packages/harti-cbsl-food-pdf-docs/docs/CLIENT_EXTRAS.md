# Client extras — `harti-cbsl-food-pdf-docs`

## Typed models

Canonical dataclass / TS types from field-coverage domains: `food_prices`.

- Python: `python/harti_cbsl_food_pdf_docs/models.py`
...
```python
from harti_cbsl_food_pdf_docs import HartiCbslFoodPdfDocsClient, iter_lab_endpoint

with HartiCbslFoodPdfDocsClient() as client:
    for page in iter_lab_endpoint(client, None, max_pages=3, page_size=50):
        print(page.page, len(page.items), page.done)
```

```ts
import { HartiCbslFoodPdfDocsClient, iterLabEndpoint } from "./src/index.js";
const client = new HartiCbslFoodPdfDocsClient();
...
```python
from harti_cbsl_food_pdf_docs import shard_groups, shard_page_numbers, shard_offsets
