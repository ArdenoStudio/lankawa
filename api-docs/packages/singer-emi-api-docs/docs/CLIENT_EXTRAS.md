# Client extras — `singer-emi-api-docs`

## Typed models

Canonical dataclass / TS types from field-coverage domains: `fuel_energy`.

- Python: `python/singer_emi_api_docs/models.py`
...
```python
from singer_emi_api_docs import SingerEmiApiDocsClient, iter_lab_endpoint

with SingerEmiApiDocsClient() as client:
    for page in iter_lab_endpoint(client, None, max_pages=3, page_size=50):
        print(page.page, len(page.items), page.done)
```

```ts
import { SingerEmiApiDocsClient, iterLabEndpoint } from "./src/index.js";
const client = new SingerEmiApiDocsClient();
...
```python
from singer_emi_api_docs import shard_groups, shard_page_numbers, shard_offsets
