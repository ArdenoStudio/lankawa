# Client extras — `softlogic-emi-park-docs`

## Typed models

Canonical dataclass / TS types from field-coverage domains: `fuel_energy`.

- Python: `python/softlogic_emi_park_docs/models.py`
...
```python
from softlogic_emi_park_docs import SoftlogicEmiParkDocsClient, iter_lab_endpoint

with SoftlogicEmiParkDocsClient() as client:
    for page in iter_lab_endpoint(client, None, max_pages=3, page_size=50):
        print(page.page, len(page.items), page.done)
```

```ts
import { SoftlogicEmiParkDocsClient, iterLabEndpoint } from "./src/index.js";
const client = new SoftlogicEmiParkDocsClient();
...
```python
from softlogic_emi_park_docs import shard_groups, shard_page_numbers, shard_offsets
