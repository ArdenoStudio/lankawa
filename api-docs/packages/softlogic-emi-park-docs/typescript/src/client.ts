/**
 * Unofficial TypeScript client — Softlogic EMI (park — heavy crawl)
 *
...
const DEFAULT_UA = "softlogic-emi-park-docs-unofficial/0.1 (+https://github.com/Cookie-Cat21/softlogic-emi-park-docs; educational; polite)";

function sleep(ms: number): Promise<void> {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

export class SoftlogicEmiParkDocsClient {
  ...
  constructor(options: ClientOptions = {}) {
    this.baseUrl = (options.baseUrl ?? "https://www.softlogic.lk").replace(/\/$/, "");
  ...
  static readonly slug = "softlogic-emi-park-docs";
  static readonly title = "Softlogic EMI (park — heavy crawl)";
  ...
  /**
   * PARK — per-SKU EMI crawl too heavy vs Singer json-get-emi.
   * @remarks PARKED — see docs; method refuses live calls by default.
   */
  async variationDetailParked(options: RequestOptions = {}): Promise<never> {
    void options;
    throw new Error("Parked endpoint: variation_detail_parked (softlogic-emi-park-docs)");
  }
}

export default SoftlogicEmiParkDocsClient;
