...
```python
from combank_api_docs import CombankApiDocsClient, iter_lab_endpoint

with CombankApiDocsClient() as client:
    for page in iter_lab_endpoint(client, None, max_pages=3, page_size=50):
        print(page.page, len(page.items), page.done)
```

```ts
import { CombankApiDocsClient, iterLabEndpoint } from "./src/index.js";
const client = new CombankApiDocsClient();
...
```python
from combank_api_docs import shard_groups, shard_page_numbers, shard_offsets
