/**
 * Unofficial TypeScript client — SL Bank Supermarket Card Offers
 *
...
const DEFAULT_UA = "sl-bank-card-offers-docs-unofficial/0.1 (+https://github.com/Cookie-Cat21/sl-bank-card-offers-docs; educational; polite)";

function sleep(ms: number): Promise<void> {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

export class SlBankCardOffersDocsClient {
  ...
  static readonly slug = "sl-bank-card-offers-docs";
  static readonly title = "SL Bank Supermarket Card Offers";
  ...
  /**
   * Aggregator pack: Sampath + HNB Venus + ComBank HTML + peers → supermarket DOW.
   * Catalog id: `pack_overview` · GET
   */
  async packOverview(options: RequestOptions = {}): Promise<unknown> {
    return this.requestJson<unknown>(this.resolveUrl("https://www.sampath.lk/api/card-promotions?category=super_markets&page_number=1&size=20", options.query), { ...options, method: "GET" });
  }
}

export default SlBankCardOffersDocsClient;
