# Client extras — `litro-laugfs-lpg-docs`

## Typed models

Canonical dataclass / TS types from field-coverage domains: `fuel_energy`.

- Python: `python/litro_laugfs_lpg_docs/models.py`
...
```python
from litro_laugfs_lpg_docs import LitroLaugfsLpgDocsClient, iter_lab_endpoint

with LitroLaugfsLpgDocsClient() as client:
    for page in iter_lab_endpoint(client, None, max_pages=3, page_size=50):
        print(page.page, len(page.items), page.done)
```

```ts
import { LitroLaugfsLpgDocsClient, iterLabEndpoint } from "./src/index.js";
const client = new LitroLaugfsLpgDocsClient();
...
```python
from litro_laugfs_lpg_docs import shard_groups, shard_page_numbers, shard_offsets
