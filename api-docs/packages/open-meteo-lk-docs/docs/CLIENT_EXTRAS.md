# Client extras — `open-meteo-lk-docs`

## Typed models

Canonical dataclass / TS types from field-coverage domains: `weather_aqi`.

- Python: `python/open_meteo_lk_docs/models.py`
...
```python
from open_meteo_lk_docs import OpenMeteoLkDocsClient, iter_lab_endpoint

with OpenMeteoLkDocsClient() as client:
    for page in iter_lab_endpoint(client, None, max_pages=3, page_size=50):
        print(page.page, len(page.items), page.done)
```

```ts
import { OpenMeteoLkDocsClient, iterLabEndpoint } from "./src/index.js";
const client = new OpenMeteoLkDocsClient();
...
```python
from open_meteo_lk_docs import shard_groups, shard_page_numbers, shard_offsets
