# Client extras — `metdept-cap-api-docs`

## Typed models

Canonical dataclass / TS types from field-coverage domains: `weather_aqi`.

- Python: `python/metdept_cap_api_docs/models.py`
...
```python
from metdept_cap_api_docs import MetdeptCapApiDocsClient, iter_lab_endpoint

with MetdeptCapApiDocsClient() as client:
    for page in iter_lab_endpoint(client, None, max_pages=3, page_size=50):
        print(page.page, len(page.items), page.done)
```

```ts
import { MetdeptCapApiDocsClient, iterLabEndpoint } from "./src/index.js";
const client = new MetdeptCapApiDocsClient();
...
```python
from metdept_cap_api_docs import shard_groups, shard_page_numbers, shard_offsets
