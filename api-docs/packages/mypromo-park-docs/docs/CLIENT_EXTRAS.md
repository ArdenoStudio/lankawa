# Client extras — `mypromo-park-docs`

## Typed models

Canonical dataclass / TS types from field-coverage domains: `card_offers`.

- Python: `python/mypromo_park_docs/models.py`
...
```python
from mypromo_park_docs import MypromoParkDocsClient, iter_lab_endpoint

with MypromoParkDocsClient() as client:
    for page in iter_lab_endpoint(client, None, max_pages=3, page_size=50):
        print(page.page, len(page.items), page.done)
```

```ts
import { MypromoParkDocsClient, iterLabEndpoint } from "./src/index.js";
const client = new MypromoParkDocsClient();
...
```python
from mypromo_park_docs import shard_groups, shard_page_numbers, shard_offsets
