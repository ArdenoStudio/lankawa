...
rates-tariff HTML FX/FD; park stale FD JSON API.
...
This package currently lives inside the Lankawa monorepo at `api-docs/packages/boc-rates-docs/`.
**Extract to** public repo `Cookie-Cat21/boc-rates-docs` using [`api-docs/EXTRACTION_PROMPT.md`](../../EXTRACTION_PROMPT.md).
...
```bash
cd api-docs/packages/boc-rates-docs
...
- Python: [`python/`](./python/) — `boc-rates-docs-unofficial` (`pip install -e .`)
- TypeScript: [`typescript/`](./typescript/) — `@cookie-cat21/boc-rates-docs-client`
