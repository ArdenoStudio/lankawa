...
```python
from sdb_rates_docs import SdbRatesDocsClient, iter_lab_endpoint

with SdbRatesDocsClient() as client:
    for page in iter_lab_endpoint(client, None, max_pages=3, page_size=50):
        print(page.page, len(page.items), page.done)
```

```ts
import { SdbRatesDocsClient, iterLabEndpoint } from "./src/index.js";
const client = new SdbRatesDocsClient();
...
```python
from sdb_rates_docs import shard_groups, shard_page_numbers, shard_offsets
