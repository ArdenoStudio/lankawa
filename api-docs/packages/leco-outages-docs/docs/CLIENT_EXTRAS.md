# Client extras — `leco-outages-docs`

## Typed models

Canonical dataclass / TS types from field-coverage domains: `utilities`.

- Python: `python/leco_outages_docs/models.py`
...
```python
from leco_outages_docs import LecoOutagesDocsClient, iter_lab_endpoint

with LecoOutagesDocsClient() as client:
    for page in iter_lab_endpoint(client, None, max_pages=3, page_size=50):
        print(page.page, len(page.items), page.done)
```

```ts
import { LecoOutagesDocsClient, iterLabEndpoint } from "./src/index.js";
const client = new LecoOutagesDocsClient();
...
```python
from leco_outages_docs import shard_groups, shard_page_numbers, shard_offsets
